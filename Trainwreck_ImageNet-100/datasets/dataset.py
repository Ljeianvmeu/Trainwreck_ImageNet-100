...
class Dataset:
    ...
    def __init__(
        ...
        elif self.dataset_id == "imagenet100":
            # ImageNet-100 image size is 224x224
            self.input_size = 224

            # Build ImageNet-100 dataset
            from torchvision.datasets import ImageFolder
            import os
            from torchvision import transforms as T

            # Define transformation matrix directly here
            # If transforms is passed in, use that, otherwise define our own
            if transforms is None:
                # Training set transformations (includes data augmentation)
                ...
                # Validation set transformations (no data augmentation)
                ...
                # Use the provided transforms
                train_transforms = transforms
                val_transforms = transforms

            # Directly use the organized train and val directories
            imagenet100_train_dir = os.path.join(root_data_dir, 'imagenet100', 'train')
            imagenet100_val_dir = os.path.join(root_data_dir, 'imagenet100', 'val')

            # Create datasets using the appropriate transformations
            self.train_dataset = ImageFolder(
                root=imagenet100_train_dir,
                transform=train_transforms
            )

            self.test_dataset = ImageFolder(
                root=imagenet100_val_dir,
    ...
    def data_loaders(
        ...
        # Special optimizations for ImageNet-100 data loading
        if self.dataset_id == "imagenet100":
            num_workers = 8  # Use multiple threads for data loading
        ...
        train_loader = DataLoader(
            ...
            shuffle=shuffle,
            pin_memory=True,
            num_workers=num_workers,  # Increase worker threads
            prefetch_factor=prefetch_factor,  # Prefetch factor
            persistent_workers=True  # Keep worker threads active
        )
