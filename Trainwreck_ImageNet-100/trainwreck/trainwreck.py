        ...
        while len(clean_classes) > 0:
            ...
            for b, (X, _) in enumerate(tqdm(data_loader)):
                ...
                for b_i, x_pert in enumerate(X_pert):
                    ...
                    # Save the image and record as processed
                    self._save_poisoned_img(x_pert, i)
                    processed_indices.add(i)

                    # Exit early if maximum modification count is reached
                    if len(processed_indices) >= self.n_max_modifications:
                        print(f"Maximum modification count {self.n_max_modifications} reached, exiting early")
                        break
